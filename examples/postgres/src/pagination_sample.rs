...
use parsql::postgres::{get_all, SqlQuery, SqlParams, FromRow};
use postgres::{Client, Error, Row, types::ToSql};
...
pub fn run_derive_pagination_examples(client: &mut Client) -> Result<(), Error> {
    ...
    let count_result = client.query_one("SELECT COUNT(*) FROM users WHERE state >= $1", &[&0_i16])?;
