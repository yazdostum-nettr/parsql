use parsql::postgres::{
    ...
    transactional::{begin, tx_delete, tx_get, tx_insert, tx_update},
};
...
pub fn transaction_with_crud_ops(client: &mut Client) -> Result<(), Error> {
    ...
    let rows_affected = tx.insert::<TxInsertUser, i64>(insert_user)?;
    ...
    let rows_affected = tx.insert::<TxInsertPost, i64>(insert_post)?;
...
pub fn transaction_with_helper_functions(client: &mut Client) -> Result<(), Error> {
    ...
    let (mut tx, rows_affected) = tx_insert::<TxInsertUser, i64>(tx, insert_user)?;
    println!("Kullanıcı eklendi, etkilenen satır: {}", rows_affected);
    ...
    let (mut tx, rows_affected) = tx_insert::<TxInsertPost, i64>(tx, insert_post)?;
    println!("Post eklendi, etkilenen satır: {}", rows_affected);
...
pub fn complex_transaction_example(client: &mut Client) -> Result<(), Error> {
    ...
    println!(
        "Admin kullanıcısı güncellendi, etkilenen satır: {}",
        admin_updated
    );

    // 4. Kullanıcı durumu 2 olan kullanıcıları 3 olarak güncelle
    let state_updated = tx.execute("UPDATE users SET state = 3 WHERE state = 2", &[])?;
...
pub fn transaction_with_delete(client: &mut Client) -> Result<(), Error> {
    ...
    let (mut tx, _) = tx_insert::<TxInsertUser, i64>(tx, insert_user)?;
    ...
    let (tx, _) = tx_insert::<TxInsertPost, i64>(tx, insert_post)?;
    ...
    let posts_deleted = tx.execute("DELETE FROM posts WHERE user_id = $1", &[&user_id])?;
